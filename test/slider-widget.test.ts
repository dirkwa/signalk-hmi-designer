...
describe('slider widget kind', () => {
  ...
  it('defaults display.decimals to the applyBind merge sentinel (1), not 0', () => {
    // applyBind's metadata merge treats decimals===1 as "still default,
    // safe to overwrite with the bound path's precision" and any other
    // value as "user set, leave alone". A slider default of 0 would look
    // user-set and silently swallow nonzero metadata precision the moment
    // the widget is bound.
    const sliderCase = APP.slice(
      APP.indexOf("case 'slider':"),
      APP.indexOf("case 'slider':") + 500
    )
    expect(sliderCase).toContain('decimals: 1')
  })
